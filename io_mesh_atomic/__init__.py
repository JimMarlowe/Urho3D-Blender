...
DEBUG = 0
if DEBUG: print("Atomic export init")

bl_info = {
    "name": "Atomic export",
    "description": "Atomic Game Engine export",
    "author": "reattiva",
    "version": (0, 6),
    "blender": (2, 66, 0),
    "location": "Properties > Render > Atomic export",
    "warning": "Grande bugs, use at your own risk",
    "wiki_url": "",
    "tracker_url": "",
...
if "decompose" in locals():
    ...
    imp.reload(export_atomic)
    imp.reload(export_scene)
    imp.reload(utils)
    if DEBUG and "testing" in locals(): imp.reload(testing)

from .decompose import TOptions, Scan
from .export_atomic import AtomicExportData, AtomicExportOptions, AtomicWriteModel, AtomicWriteAnimation, \
                         AtomicWriteTriggers, AtomicExport
from .export_scene import SOptions, AtomicScene, AtomicExportScene, AtomicWriteMaterial, AtomicWriteMaterialsList
from .utils import PathType, FOptions, GetFilepath, CheckFilepath, ErrorsMem
if DEBUG: from .testing import PrintAtomicData, PrintAll
...
# under the Atomic exporter addon row
class AtomicAddonPreferences(bpy.types.AddonPreferences):
    bl_idname = __name__
    ...
    objectsPath = StringProperty(
            name = "Default Prefabs subpath",
            description = "Prefabs subpath (relative to output)",
            default = "Prefabs")
...
class AtomicExportSettings(bpy.types.PropertyGroup):
    ...
    prefabs = BoolProperty(
            name = "Export Atomic Prefabs",
            description = "Exports Atomic prefab objects",
            default = False,
            update = update_func)
    ...
    scenePrefab = BoolProperty(
            name = "Scene Prefab",
            description = "Same content as 'Collective', but outputs a Atomic scene (with Octree, PhysicsWorld and DebugRenderer)",
            default = False,
            update = update_func)
...
class AtomicExportResetOperator(bpy.types.Operator):
    """ Reset export settings """

    bl_idname = "atomic.exportreset"
    ...
    def execute(self, context):
        context.scene.atomic_exportsettings.reset(context)
        return {'FINISHED'}


# Reset output paths button
class AtomicExportResetPathsOperator(bpy.types.Operator):
    """ Reset paths """

    bl_idname = "atomic.exportresetpaths"
    ...
    def execute(self, context):
        context.scene.atomic_exportsettings.reset_paths(context, True)
        return {'FINISHED'}


# View log button
class AtomicReportDialog(bpy.types.Operator):
    """ View export log """
    
    bl_idname = "atomic.report"
    bl_label = "Atomic export report"
...
# Export button
class AtomicExportOperator(bpy.types.Operator):
    """ Start exporting """
    
    bl_idname = "atomic.export"
    bl_label = "Export"
...
class AtomicExportRenderPanel(bpy.types.Panel):
    
    bl_idname = "atomic.exportrenderpanel"
    bl_label = "Atomic export"
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.label("Settings:")
        row.operator("atomic.exportreset", text="", icon='LIBRARY_DATA_DIRECT')
...
def PostLoad(dummy):
    ...
    settings = bpy.context.scene.atomic_exportsettings
...
def register():
    if DEBUG: print("Atomic export register")
    
    #bpy.utils.register_module(__name__)
        
    bpy.utils.register_class(AtomicAddonPreferences)
    bpy.utils.register_class(AtomicExportSettings)
    bpy.utils.register_class(AtomicExportOperator)
    bpy.utils.register_class(AtomicExportResetOperator)
    bpy.utils.register_class(AtomicExportResetPathsOperator)
    bpy.utils.register_class(AtomicExportRenderPanel)
    bpy.utils.register_class(AtomicReportDialog)
    
    bpy.types.Scene.atomic_exportsettings = bpy.props.PointerProperty(type=AtomicExportSettings)
...
def unregister():
    if DEBUG: print("Atomic export unregister")
    
    #bpy.utils.unregister_module(__name__)
    
    bpy.utils.unregister_class(AtomicAddonPreferences)
    bpy.utils.unregister_class(AtomicExportSettings)
    bpy.utils.unregister_class(AtomicExportOperator)
    bpy.utils.unregister_class(AtomicExportResetOperator)
    bpy.utils.unregister_class(AtomicExportResetPathsOperator)    
    bpy.utils.unregister_class(AtomicExportRenderPanel)
    bpy.utils.unregister_class(AtomicReportDialog)
    
    del bpy.types.Scene.atomic_exportsettings
...
def ExecuteAtomicExport(context):
    global logList
    ...
    settings = context.scene.atomic_exportsettings
    ...
    # Scene export data
    uScene = AtomicScene(context.scene)
    ...
    for tData in tDataList:
        ...
        uExportData = AtomicExportData()
        
        uExportOptions = AtomicExportOptions()
        ...
        if DEBUG: ttt = time.time() #!TIME
        AtomicExport(tData, uExportOptions, uExportData, settings.errorsMem)
        ...
        for uModel in uExportData.models:
            if uModel.geometries:
                ...
                if CheckFilepath(filepath[0], fOptions):
                    ...
                    AtomicWriteModel(uModel, filepath[0]) 
        ...
        for uAnimation in uExportData.animations:
            ...
            if CheckFilepath(filepath[0], fOptions):
                ...
                AtomicWriteAnimation(uAnimation, filepath[0])
            ...
            if uAnimation.triggers:
                ...
                if CheckFilepath(filepath[0], fOptions):
                    ...
                    AtomicWriteTriggers(uAnimation.triggers, filepath[0], fOptions)
        ...
        if settings.materials:
            for uMaterial in uExportData.materials:
                ...
                if CheckFilepath(filepath[0], fOptions):
                    ...
                    AtomicWriteMaterial(uScene, uMaterial, filepath[0], fOptions)
            ...
            if settings.materialsList:
                for uModel in uExportData.models:
                    ...
                    if CheckFilepath(filepath[0], fOptions):
                        ...
                        AtomicWriteMaterialsList(uScene, uModel, filepath[0])
    ...
    if settings.prefabs:
        AtomicExportScene(context, uScene, sOptions, fOptions)
...
def ExecuteAddon(context):
    ...
    startTime = time.time()
    print("----------------------Atomic export start----------------------")    
    ExecuteAtomicExport(context)
    log.setLevel(logging.DEBUG)
    log.info("Export ended in {:.4f} sec".format(time.time() - startTime) )
    
    bpy.ops.atomic.report('INVOKE_DEFAULT')
