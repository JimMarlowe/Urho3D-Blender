...
# Based upon the Urho3D Importer
#
...
class AtomicModel:
    def __init__(self):
        # Model name
        self.name = None
        # List of AtomicVertexBuffer
        self.vertexBuffers = []
        # List of AtomicIndexBuffer
        self.indexBuffers = []
        # List of AtomicGeometry
        self.geometries = []
        # List of AtomicVertexMorph
        self.morphs = []
        # List of AtomicBone
        self.bones = []
...
class AtomicExportData:
    def __init__(self):
        # List of AtomicModel
        self.models = []
        # List of AtomicAnimation
        self.animations = []
        # List of AtomicMaterial
        self.materials = []
        
class AtomicExportOptions:
    def __init__(self):
...
def AtomicWriteModel(model, filename):
...
def AtomicWriteAnimation(animation, filename):
...
def AtomicWriteTriggers(triggersList, filename, fOptions):
...
def AtomicExport(tData, uExportOptions, uExportData, errorsMem):
    ...
    # Atomic lod vertex buffer
    vertexBuffer = None
    # Atomic lod index buffer
    indexBuffer = None
    # Maps old vertex index to Atomic vertex buffer index and Atomic vertex index
    modelIndexMap = {}
    
    # For each geometry
    for tGeometry in tData.geometriesList:
        
        uGeometry = AtomicGeometry()
        ...
        for lodIndex, tLodLevel in enumerate(tGeometry.lodLevels):
            ...
            if vertexBuffer is None or (lodIndex == 0 and not useOneBuffer):
                vertexBuffer = AtomicVertexBuffer()
            ...
            # Maps old vertex index to new vertex index in the new Atomic buffer
            indexMap = {}
    ...
    for tMorph in tData.morphsList:
        ...
        for tVertexIndex in randomIndicesAll:
            # Get the correspondent Atomic vertex buffer and vertex index (there can be more than one)
        ...
        for tVertexIndex, tMorphVertex in tMorph.vertexMap.items():
            ...
            for uVertexBufferIndex, uVertexIndex in vbviSet:
                ...
                try:
                    ...
                    uMorphVertexBuffer = AtomicVertexBuffer()
                ...
                uMorphVertex = AtomicVertex(tMorphVertex)
                try:
    ...
    for tAnimation in tData.animationsList:
        ...
        for tTrack in tAnimation.tracks:
            ...
            for tFrame in tTrack.frames:
                uKeyframe = AtomicKeyframe(tFrame)
                try:
